...
if (
    templateGray.shape[0] > imageGray.shape[0]
    or templateGray.shape[1] > imageGray.shape[1]
):
    scale_factor = min(
        imageGray.shape[1] / templateGray.shape[1],
        imageGray.shape[0] / templateGray.shape[0],
    )
    new_size = (
        int(templateGray.shape[1] * scale_factor),
        int(templateGray.shape[0] * scale_factor),
    )
...
for method in methods:
    ...
    cv2.imshow(f"Metoda {method}, maxVal={maxVal}", clone)
    cv2.waitKey(0)
